  ...
  orionPagesUpdateForm: {
    before: {
      update: function(doc) {
        ...
        //console.log("update Hook",self,doc,updatingPage,name);
        if (!name) {
          self.result(false);
        } else {
          doc = orion.pages.templates[name].schema.clean(doc, {
...
ReactiveTemplates.onCreated('pages.update', function() {
  ...
  const pagesHandle=self.subscribe('pageById', RouterLayer.getParam('_id'));
  self.autorun(function() {
    const pagesIsReady = pagesHandle.ready();
    self.item=orion.pages.collection.findOne(RouterLayer.getParam('_id'));

    console.log(`pageById is ${pagesIsReady ? 'ready' : 'not ready'}`);
    //console.log("pagesclient autorun...",RouterLayer.getParam('_id'));
  });
});
...
ReactiveTemplates.helpers('pages.update', {
  getSchema: function() {
    ...
    var item2 = orion.pages.collection.findOne(RouterLayer.getParam('_id'));
    //console.log("pages.update getSchema HELPER...",item2);
    return item2 && orion.pages.templates[item2.template] && orion.pages.templates[item2.template].schema;
  },

  item: function() {
    var foundItem=orion.pages.collection.findOne(RouterLayer.getParam('_id'));
    //console.log("pages.item item HELPER...", foundItem );
    return foundItem;
  },
});
...
Template.orionPages_mainTemplate.helpers({
  page: function() {
    //console.log("orionPages_mainTemplate", orion);
